...
1. Locate the report.
   ...
   - Require `agent-report.json`, `audit-report.json`, and `run-manifest.json`.
...
3. Choose the smallest useful depth from `agent-report.json`.
...
6. Look for gaps when using `standard` or `deep`.
...
7. Return a concise human review.
