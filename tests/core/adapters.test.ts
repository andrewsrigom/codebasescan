...
test('Semgrep normalization preserves source severity without inventing CVSS', () => {
  const findings = normalizeSemgrep({ results: [{ check_id: 'example.rule', path: 'src/example.ts', start: { line: 1 }, extra: { severity: 'ERROR', message: 'Review this sink.' } }] }, snapshotOf('eval(input)'));
...
test('scanner paths cannot escape the snapshot', () => {
  const result = normalizeSemgrep({ results: [{ path: '../../outside.ts', start: { line: 1 }, extra: { severity: 'ERROR' } }] }, snapshotOf('eval(input)'));
...
test('Gitleaks normalization discards raw secrets and matched lines', () => {
  const findings = normalizeGitleaks([{ Description: 'Example secret', RuleID: 'example', File: 'src/example.ts', StartLine: 1, Secret: 'sensitive-fixture-value', Match: 'sensitive-fixture-value' }], snapshotOf('const token = "sensitive-fixture-value";'));
...
test('out-of-bounds scanner locations cannot fabricate source evidence', () => {
  ...
  for (const line of [0, -1, 999, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
    assert.equal(normalizeSemgrep({ results: [{ check_id: 'bad-location', path: 'src/example.ts', start: { line }, extra: { severity: 'ERROR' } }] }, source).length, 0);
    assert.equal(normalizeGitleaks([{ File: 'src/example.ts', StartLine: line }], source).length, 0);
  }
});
