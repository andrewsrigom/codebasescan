...
test('LangGraph fans in scanner results and pauses for publication review', async (context) => {
  ...
  assert.equal(store.audit(audit.id).report?.scanners.length, 22);
  assert.ok(
  ...
  assert.ok(
    store.audit(audit.id).report?.scanners.some((scanner) => scanner.id === 'saas-security'),
  );
  ...
  assert.equal(store.audit(audit.id).report?.schemaVersion, 10);
