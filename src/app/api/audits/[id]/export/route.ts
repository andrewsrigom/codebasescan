...
export async function GET(request: Request, { params }: {
  ...
  try {
    ...
    if (!report)
      return new Response('No report is available yet.', { status: 409 });
    const format = new URL(request.url).searchParams.get('format') ?? 'json';
    if (!['json', 'md', 'html', 'sarif'].includes(format))
      return new Response('Unsupported export format.', { status: 400 });
    const content = format === 'html' ? toHtml(report) : format === 'md' ? toMarkdown(report) : JSON.stringify(format === 'sarif' ? toSarif(report) : report, null, 2);
    return new Response(content, {
      headers: {
        'Content-Type': format === 'html' ? 'text/html; charset=utf-8' : format === 'md' ? 'text/markdown; charset=utf-8' : 'application/json; charset=utf-8',
