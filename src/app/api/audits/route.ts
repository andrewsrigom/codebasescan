...
import { record, uuid } from '../../../domain/validation.ts';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const audit = store().enqueue(uuid(input.projectId));
    return Response.json({ id: audit.id }, { status: 201 });
  }
  catch (cause) {
    return Response.json({ error: cause instanceof Error ? cause.message : 'Could not queue audit.' }, { status: 400 });
  }
}
