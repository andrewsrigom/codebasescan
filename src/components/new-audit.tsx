...
export async function mutate(url: string, body: unknown): Promise<unknown> {
  const response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Traceward-Client': 'local-ui' }, body: JSON.stringify(body) });
...
export function NewAudit({ projects }: {
  ...
  async function start() {
    ...
    setError('');
    try {
      const payload = await mutate('/api/audits', { projectId: selected }) as {
        id: string;
      };
  ...
  return <>
    ...
    <dialog className="modal" ref={dialog} aria-labelledby="new-audit-title">
      <div className="modal-head">
        <h2 id="new-audit-title">
          Start a security review
        </h2>
        <button className="icon-button" aria-label="Close dialog" onClick={() => dialog.current?.close()}>
          <Icon name="close" />
        </button>
      </div>
      <p>
        Only projects explicitly registered through the local CLI can be scanned.
      </p>
      {projects.length ? <>
        <label htmlFor="project-select">
          Project
        </label>
        <select id="project-select" value={selected} onChange={(event) => setSelected(event.target.value)}>
        ...
        <button className="button primary full" disabled={pending || !selected} onClick={start}>
