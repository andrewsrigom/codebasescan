...
process.umask(0o077);
const config = { ...configuration(), aiMode: 'disabled' as const, semgrep: false, gitleaks: false };
...
try {
  ...
  const root = await validateProjectRoot(path.resolve('fixtures/review-worthy-saas'), config.dataDirectory);
  ...
  if (process.argv.includes('--review') && store.audit(audit.id).status === 'awaiting_review') {
    store.publish(audit.id, 'Demo fixture reviewed for demonstration. All unverified findings remain unresolved.');
    store.claim(audit.id);
    await executeAudit(store, audit.id, config);
  }
  console.log(`Demo audit: ${audit.id}`);
  console.log(`Status: ${store.audit(audit.id).status}`);
  console.log('Run npm run dev, then open http://127.0.0.1:3000. Start npm run worker before submitting publication review.');
}
finally {
